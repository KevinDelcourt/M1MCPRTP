/*
 * Producteur-consommateur, variante 4 les producteurs effectuent 2 depots consécutifs
 * 
...
typedef struct
{
    ...
    int nbVide;
    int nbProdEnAttente;
    int rangProdPrioritaire; //Si il existe un producteur n'ayant déposé qu'un message, vaut -1 sinon
...
pthread_cond_t condDepotPriorite = PTHREAD_COND_INITIALIZER;
...
void initialiserVarPartagees(void)
{
    ...
    resCritiques.rangProdPrioritaire = -1;
    resCritiques.nbProdEnAttente = 0;
...
void afficherBuffer(void)
{
    ...
    for (i = 0; i < nbCases; i++)
    {
        printf("\tCase %d : T%s", i, resCritiques.buffer[i].info);
...
{
    int etat;
    while (resCritiques.nbVide == 0 && resCritiques.rangProdPrioritaire == rangProd)
    {
#ifdef TRACE_SOUHAIT
        printf("\t\t\tProd %d: \033[0;33m En attente\033[0m dans la condition condDepot Prioritaire\n",
               rangProd);
#endif
        if (0 != (etat = pthread_cond_wait(&condDepotPriorite, &exclusionMutuelleMoniteur)))
            thdErreur(etat, "Wait condition depot prioritaire", etat);
#ifdef TRACE_SOUHAIT
        printf("\t\t\tProd %d: \033[0;32m Sort\033[0m de la condition condDepot Prioritaire\n",
               rangProd);
#endif
    }

    while (resCritiques.nbVide == 0 || (resCritiques.rangProdPrioritaire != -1 && resCritiques.rangProdPrioritaire != rangProd))
    {
#ifdef TRACE_SOUHAIT
        printf("\t\t\tProd %d: \033[0;33m En attente\033[0m dans la condition condDepot\n",
               rangProd);
#endif
        resCritiques.nbProdEnAttente++;
    ...
    if (resCritiques.rangProdPrioritaire == -1)
    {
        resCritiques.rangProdPrioritaire = rangProd;
        if (0 != (etat = pthread_cond_signal(&condRetrait)))
            thdErreur(etat, "Signal condition retrait", etat);
    }
    else //(resCritiques.rangProdPrioritaire == rangProd) -> fin du 2eme passage
    {
        resCritiques.rangProdPrioritaire = -1;
        if (resCritiques.nbProdEnAttente > 0 && resCritiques.nbVide > 0)
        {
...
void retirer(TypeMessage *unMessage, int rangConso)
{
    ...
    if (resCritiques.rangProdPrioritaire != -1)
    {
        if (0 != (etat = pthread_cond_signal(&condDepotPriorite)))
            thdErreur(etat, "Signal condition depot prioritaire", etat);
    }
    else
    {
        if (0 != (etat = pthread_cond_signal(&condDepot)))
